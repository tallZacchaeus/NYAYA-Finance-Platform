...
export default function TeamLeadDashboard() {
  ...
  // Submitted = awaiting finance review; finance_reviewed = awaiting SATGO approval
  ...
  return (
    <div className="p-5 sm:p-7 space-y-7">
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <h1 className="font-display text-2xl text-[#F5E8D3]">Team Lead Dashboard</h1>
        <p className="font-body text-sm text-[#A89FB8] mt-0.5">Review and recommend your department's requests</p>
      </motion.div>

      {/* Stat cards */}
      <StaggerList className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StaggerItem>
          <StatCard title="Submitted"     value={pending.length}     icon={<Clock       className="w-4 h-4" />} accentColor="#FBBF24" />
        </StaggerItem>
        <StaggerItem>
      ...
      {pendingAmt > 0 && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-body text-xs text-[#A89FB8]">Total value awaiting your review</p>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h2 className="font-display text-lg text-[#F5E8D3]">Submitted — Awaiting Finance Review</h2>
            {pending.length > 0 && (
        ...
        {isLoading ? (
          ...
          <div className="rounded-xl border border-[#2D1A73] bg-[#13093B]">
            <EmptyState
              title="All caught up"
              description="No submitted requests are currently awaiting finance review."
            ...
            <table className="w-full text-sm font-body">
              <thead>
                <tr className="bg-[#13093B] border-b border-[#2D1A73]">
                  {['Reference', 'Title', 'Amount', 'Requester', 'Status', ''].map(h => (
      ...
      <div className="pt-1">
        <Link
          ...
          className="inline-flex items-center gap-2 font-body text-sm text-[#A89FB8] hover:text-[#D4A843] transition-colors"
        >
